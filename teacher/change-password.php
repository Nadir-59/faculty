...
<?php 
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(!empty($old_pass) && !empty($new_pass) && !empty($confirm_pass) ){
            ...
            $check_old_pass = "SELECT password FROM teacher WHERE email = '$_SESSION[email_emp]' && password = '$old_pass' ";
            $result = mysqli_query($conn , $check_old_pass);
            if( mysqli_num_rows($result) > 0 ){
               
                if( $new_pass === $confirm_pass ){
                  
                    $change_pass_query = "UPDATE teacher SET password = '$new_pass' WHERE email = '$_SESSION[email_emp]' ";
